...
def write_hopping(t,f,nsites,pbc):
    print >>f, '{0:.12e}'.format(t), 1,2,0,0
    if (pbc):
       print >>f, '{0:.12e}'.format(t), 1,nsites,0,0
    for i in range(2,nsites):
      print >>f, '{0:.12e}'.format(t), i,i-1,0,0
      print >>f, '{0:.12e}'.format(t), i,i+1,0,0
    if (pbc):
       print >>f, '{0:.12e}'.format(t), nsites,1,0,0
    print >>f, '{0:.12e}'.format(t), nsites,nsites-1,0,0

def run(nsites, t, U, fname, pbc):
...
if __name__=="__main__":
    import sys
    run(int(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), sys.argv[4], eval(sys.argv[5]))
